...
function [OUT1,OUT2] = testc(a,w,type,label)
    ...
    elseif (isdatafile(a)) % datafile needs some handling as we need to process all objects
	    c = getsize(a,3);
	    out2 = zeros(1,c);
	    next = 1;
...
switch type
	...
	case 'soft'  % normalised difference between desired and real targets
		b = seldat(a,n);
		b = setlabtype(b*classc,'soft');
		k = findfeatlab(a,label);
		d = abs(+b(:,k)-gettargets(b));
		J = find(isnan(d));
		d(J) = ones(size(J));
		%out1 = sum(d)/2; % seems needed for consistency, but why /2 ????
		out1 = sum(d);
		out2 = [];
