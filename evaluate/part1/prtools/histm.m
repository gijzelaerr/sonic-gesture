...
% bin are empty for A. All feature values of a new dataset B that are outside 
% the histogram defined on A are mapped into these bins.
%
...
if fixed                  %  fixed histograms
	...
	if isempty(a)
		...
		w = mapping(mfilename,'trained',bins,[],0,length(bins));
		w = setname(w,mapname);
	else
		n = getfeatsize(a,3);
		if isdatafile(a)
