...
function [w,slack]=lessc(a,C,kernel,estimate,normalize,combiner,combinerStyle)
...
if nargin < 1 | isempty(a)
    w=mapping(mfilename,{C,kernel,estimate,normalize,combiner,combinerStyle});
    w=setname(w,'LESS(mean) classifier');
    return
end
...
if ~ismapping(C)   % train the mapping
    ...
    if numClasses>2 && strcmp(lower(combinerStyle),'indep')
        w=mclassc(a,mapping(mfilename,{C,kernel,estimate,normalize}),combiner);
        return;
    end
    ...
    [d.beta,d.M,d.S,slack]=less(a,C,kernel,estimate,combiner,combinerStyle);
    d.kernel=kernel;
    d.combiner=combiner;
