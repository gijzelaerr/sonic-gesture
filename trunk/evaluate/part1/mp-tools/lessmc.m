...
if ~ismapping(C)   % train the mapping
    ...
    if length(C)==1
        ...
        for i=1:numClasses
            C(i)=C(i)/classObjects(i)*numObjects/numClasses;
        end
        ...
        for i=1:numClasses-1
            for j=i+1:numClasses
                ...
                if strcmp(lower(combinerStyle),'flex')
                    ...
                    fvector=[fvector; repmat((classObjects(i)+classObjects(j))/numObjects,numFeatures,1)];
                end
            end
        end
    case 'single'
...
%        f(numDimensions+1:end)=Cvector;
    ...
    case 'multi'
        ...
        for i=1:numClasses-1
            for j=i+1:numClasses
                ...
                v=mapmeans(x(IX,:),M([i j],:),S([i j],:),vars);
        
                num=classObjects(i)+classObjects(j);

                y=[ones(classObjects(i),1);-ones(classObjects(j),1)];

                A(constraintsOffset+(1:num),dimensionsOffset+(1:numFeatures))=v.*repmat(y,1,numFeatures);
    ...
    case 'single'
        ...
        for i=1:numClasses
            ...
            v=mapmeans(x([IX1;IX2],:),M([i numClasses+i],:),S([i numClasses+i],:),vars);

            y=[ones(classObjects(i),1);-ones(numObjects-classObjects(i),1)];
                
            A(constraintsOffset+(1:numObjects),dimensionsOffset+(1:numFeatures))=v.*repmat(y,1,numFeatures);
