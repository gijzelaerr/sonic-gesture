...
% remaining part is used for testing. This is rotated over all parts. ERR is 
...
function [err,cerr,nlabout,tclassf] = crossval(data,classf,n,nrep,testf,fid)
	...
	if iscell(classf) | iscell(data)
			...
			prwaitbar(numd,s2);
 			...
 			for jd = 1:numd % Run over a set of datasets
				...
				if nargout > 3 % store resulting classifiers
					[ee,cc,nn,tclassf(:,:,jc,jd)] = feval(mfilename,data{jd},classf{jc},n,nrep,testf);
				else
		...
		if nrep > 1
			...
			for j=1:nrep
				...
				if nargout > 3
					[ee(j),ss,nlabout,tclassf(:,j,1,1)] = feval(mfilename,data,classf,n,1,testf);
				else
